...
const CityCards = (props) => {
  ...
  const [cityUpdated, setCityUpdated] = useState(data.name)
    ...
    if(cityName === cityUpdated){
      ...
      try {
        const response = await fetch(
          ...
          state.URL_API.city + 
          `/?idCity=${data.cityId}&nameToUpdate=${cityName}`,{
            method: "PUT",
            headers: {
    ...
    <section className='city'>
      ...
      <div className="city-actions">
        ...
        <div className="delete">
          <ModalDeleteCity deleteId={deleteId} id={data.cityId} cityName={data.name}/>
        </div>
      </div>
    </section>
  )
